...
-- UI References (Global scope for event handlers)
local PlayerInput
        ...
        if targetPlayer ~= player then
            ...
            if playerName == lowerTargetName or playerName:find(lowerTargetName, 1, true) then
                if targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    foundPlayer = targetPlayer -- Remember for later
                end
            end
        end
    end
    
    -- Tier 2: Workspace Models (For games that store characters directly)
        ...
        if obj:IsA("Model") and obj.Name ~= player.Name then
            ...
            if objName == lowerTargetName or objName:find(lowerTargetName, 1, true) then
                if obj:FindFirstChild("HumanoidRootPart") and obj:FindFirstChild("Humanoid") then
                    return nil, obj -- Return character model directly
                end
            end
        end
    end
    
    -- Tier 3: Characters Folder (For games with dedicated character folders)
...
local function teleportToPlayer(targetPlayerName)
    ...
    if foundCharacter and foundCharacter:FindFirstChild("HumanoidRootPart") then
        ...
        local safeCFrame = targetPos + Vector3.new(0, 3, 0) -- Slight offset to avoid collision
...
local function updatePlayerList()
    pcall(function()
        ...
        -- Add list layout for better organization
        ...
        -- Source 2: Workspace Models (for special game structures)
        for _, obj in pairs(workspace:GetChildren()) do
            if obj:IsA("Model") and obj.Name ~= player.Name and obj:FindFirstChild("HumanoidRootPart") then
                -- Check if this isn't already in our players list
        ...
        if charFolder then
            for _, charModel in pairs(charFolder:GetChildren()) do
                if charModel:IsA("Model") and charModel.Name ~= player.Name and charModel:FindFirstChild("HumanoidRootPart") then
                    -- Check if this isn't already in our list
                    local alreadyExists = false
                    for _, existingPlayer in pairs(allPlayers) do
        ...
        for _, playerData in pairs(allPlayers) do
            ...
            -- Color coding based on status and source
...
local function createCompleteGUI()
    ...
    ZayrosFISHIT = Instance.new("ScreenGui")
    ...
    -- Enhanced Player Input with Auto-complete
    PlayerInput = Instance.new("TextBox")
    PlayerInput.Name = "PlayerInput"
    PlayerInput.Parent = TPPlayer
    PlayerInput.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
    PlayerInput.BorderSizePixel = 0
    PlayerInput.Position = UDim2.new(0.450, 0, 0.150, 0)
    PlayerInput.Size = UDim2.new(0.300, 0, 0.700, 0)
    ...
    -- Enhanced Teleport Button
    local TPPlayerButton = Instance.new("TextButton")
    TPPlayerButton.Name = "TPPlayerButton"
    TPPlayerButton.Parent = TPPlayer
    TPPlayerButton.BackgroundColor3 = Color3.fromRGB(0, 162, 255)
    TPPlayerButton.BorderSizePixel = 0
    TPPlayerButton.Position = UDim2.new(0.770, 0, 0.150, 0)
    TPPlayerButton.Size = UDim2.new(0.200, 0, 0.700, 0)
    ...
    -- Auto-updating Player List (Always Visible)
    ...
    ListOfTpPlayer.BackgroundTransparency = 0.3
    ListOfTpPlayer.BorderSizePixel = 0
    ListOfTpPlayer.Position = UDim2.new(0.040, 0, 0.170, 0)
    ListOfTpPlayer.Size = UDim2.new(0.898, 0, 0.350, 0)
    ListOfTpPlayer.Visible = true -- Always visible now
    ListOfTpPlayer.AutomaticCanvasSize = Enum.AutomaticSize.Y
    ListOfTpPlayer.ScrollBarThickness = 8
    
    local listPlayerCorner = Instance.new("UICorner")
    listPlayerCorner.Parent = ListOfTpPlayer
    
    -- Player count label
    PlayerCountLabel = Instance.new("TextLabel")
    PlayerCountLabel.Name = "PlayerCountLabel"
    PlayerCountLabel.Parent = Teleport
    PlayerCountLabel.BackgroundTransparency = 1
    PlayerCountLabel.Position = UDim2.new(0.040, 0, 0.530, 0)
    PlayerCountLabel.Size = UDim2.new(0.400, 0, 0.040, 0)
    ...
    -- TP Player button
    -- Enhanced Teleport Player Button Event
    connections[#connections + 1] = TPPlayerButton.MouseButton1Click:Connect(function()
        local targetName = PlayerInput.Text:gsub("^%s*(.-)%s*$", "%1") -- Trim whitespace
        teleportToPlayer(targetName)
    end)
    
    -- Auto-complete and Enter key support for player input
    ...
    -- Real-time player suggestions as you type
    connections[#connections + 1] = PlayerInput:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        if inputText ~= "" then
            -- Highlight matching players in the list
            for _, child in pairs(ListOfTpPlayer:GetChildren()) do
                if child:IsA("TextButton") then
                    local playerName = string.lower(child.Name)
                    if playerName:find(inputText, 1, true) then
                        child.BackgroundColor3 = Color3.fromRGB(0, 162, 255) -- Highlight match
    ...
    -- Remove old TP Island button handler (we'll keep islands as is)
    ...
    -- Periodic update to catch character spawns/respawns  
    ...
    -- Character spawn detection for new players
    connections[#connections + 1] = Players.PlayerAdded:Connect(function(newPlayer)
    ...
    -- Monitor existing players' character spawns
    for _, existingPlayer in pairs(Players:GetPlayers()) do
